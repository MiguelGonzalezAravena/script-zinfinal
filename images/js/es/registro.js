...
//					return this.show_status($, 'error', 'La contrase&ntilde;a tiene que ser distinta que el nick');
				...
				$.ajax({
					type: 'POST',
					url: '/registro-check-nick.php',
					data: 'nick='+encodeURIComponent(value),
					success: function(h){
			...
			case 'password':
				...
				if($.inArray(value.toLowerCase(),this.banned_passwords)!=-1)
					return this.show_status(el, 'error', 'Ingresa una contrase̱a m&aacute;s segura');
			...
			case 'pais':
				...
				this.hide_status($('#RegistroForm #provincia'), 'empty', 'OK');
				$('#RegistroForm #provincia').attr('disabled', 'disabled').val('');
				this.datos['ciudad'] = '';
				...
				$('#RegistroForm .pasoDos #provincia').html('').append('<option value="" selected="selected">Regi&oacute;n</option>');

				//Compruebo si ya esta en uso
				this.show_status($('#RegistroForm .pasoDos #provincia'), 'loading', 'Obteniendo...');
				$.ajax({
					type: 'GET',
					url: '/registro-geo.php',
					data: 'type=provincias&pais_code=' + value,
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(3));
								break;
							case '1': //OK
								registro.no_requerido['provincia'] = false;
								registro.show_status(el, 'ok', 'OK');
								$('#RegistroForm .pasoDos #provincia').append(h.substring(3)).removeAttr('disabled').val('').focus();
								break;
							case '2': //El pais no tiene provincias
								registro.no_requerido['provincia'] = true;
								registro.show_status(el, 'ok', 'OK');
								registro.hide_status($('#RegistroForm #provincia'), 'empty', 'OK');
								//Autocomplete para las ciudades
								$('#RegistroForm .pasoDos #ciudad').setOptions({
									extraParams: {'type':'ciudades', 'pais_code':registro.datos['pais']}
								}).flushCache();
			...
			case 'provincia':
				...
				$.ajax({
					type: 'GET',
					url: '/registro-geo.php',
					data: 'type=hay_ciudades&pais_code=' + this.datos['pais'] + '&provincia=' + value,
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(3));
							...
							case '1': //OK
								...
								//Autocomplete para las ciudades
								$('#RegistroForm .pasoDos #ciudad').setOptions({
									extraParams: {'type':'ciudades', 'pais_code':registro.datos['pais'], 'provincia':registro.datos['provincia']}
								}).flushCache();
			...
			case 'ciudad':
				...
				value = value.toLowerCase();
				//Si ya paso por aca y no hubieron cambios, devuelvo el mismo status
				if(!force_check && value == this.datos[campo+'_text'] && this.datos[campo] === this.datos[campo+'_id'])
					if(this.datos_status[campo]=='empty')
						return no_empty ? this.show_status(el, this.datos_status[campo], this.datos_text[campo]) : this.hide_status(el, this.datos_status[campo], this.datos_text[campo]);
					else
						return this.show_status(el, this.datos_status[campo], this.datos_text[campo]);
				...
				if(value != this.datos[campo+'_text']){
					this.datos[campo] = '';
					this.datos[campo+'_id'] = false;
					this.datos[campo+'_text'] = false;
					return this.show_status(el, 'error', 'La ciudad es incorrecta');
				}

				//Almaceno el dato
				this.datos[campo] = this.datos[campo+'_id'];
				...
				$.ajax({
					type: 'GET',
					url: '/registro-geo.php',
					data: 'type=check&pais_code=' + this.datos['pais'] + '&provincia=' + this.datos['provincia'] + '&ciudad=' + this.datos[campo+'_id'],
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(3));
			...
			success: function(h){
				...
				switch(h.substring(0, strpos(h, ':'))){
					...
					case 'provincia': //Provincia
						registro.change_paso(2, true);
						registro.show_status($('#RegistroForm #provincia'), 'error', h.substring(strpos(h, ':')+2));
						break;
					case 'ciudad': //Ciudad
						registro.change_paso(2, true);
