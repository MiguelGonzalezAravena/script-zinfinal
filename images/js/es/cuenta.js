$(document).ready(function(){
	...
	if (typeof $.browser.msie != 'undefined' && $.browser.version == '6.0') $('li.local-file > div.mini-modal').html('<div class="dialog-m"></div><span>Esta función no es compatible con su navegador</span>');
});
...
var cuenta = {
	...
	chgtab: function (obj) {
		$('div.tabbed-d > ul.menu-tab > li.active').removeClass('active');
	...
	chgpais: function(){
		...
		//No se selecciono ningun pais. Deshabilito provincia y ciudad
		if(empty(pais)){
			$('form[name=editarcuenta] select[name=provincia], form[name=editarcuenta] input[name=ciudad]').addClass('disabled').attr('disabled', 'disabled').val('');
			
		}else{
			//Obtengo las provincias
			$(el_provincia).html('').append('<option value="" selected="selected">Regi&oacute;n</option>');
			$.ajax({
				type: 'GET',
				url: '/registro-geo.php',
				data: 'type=provincias&pais_code=' + pais,
				success: function(h){
					switch(h.charAt(0)){
						case '0': //Error
							...
							break;
						case '1': //OK
							cuenta.no_requerido['provincia'] = false;
							$(el_provincia).append(h.substring(3)).removeAttr('disabled').val('').focus();
							break;
						case '2': //El pais no tiene provincias
							cuenta.no_requerido['provincia'] = true;
							//Autocomplete para las ciudades
							$('form[name=editarcuenta] input[name=ciudad]').setOptions({
								extraParams: {'type':'ciudades', 'pais_code': pais}
							}).flushCache();
							$(el_provincia).addClass('disabled').attr('disabled', 'disabled').val('').focus();
							break;
					}
				},
				error: function(){
	...
	next: function (isprofile) {
		...
		else $('div.tabbed-d > ul.menu-tab > li.active').next().children().click();
	},
	...
	save: function (secc, next) {
		...
		if(secc==1 && this.ciudad_id){
			if($('input[name=ciudad]').val().toLowerCase() != this.ciudad_text.toLowerCase()){
				this.ciudad_id = '';
				this.ciudad_text = '';
			}else
				params.push('ciudad='+this.ciudad_id);
		}
		...
		$.ajax({
			type: 'post', url: '/cuenta.php', data: params.join('&')+gget('key'), dataType: 'json',
			success: function (r) {
				if (r.error) {
					...
					cuenta.alert(secc, 'Los cambios fueron aceptados y serán aplicados en los próximos minutos.')
				}
	...
	imagen: {
		...
		add: function (obj) {
			...
			$.ajax({
				type: 'post', url: '/cuenta.php', data: 'ajax=1&save=7&action=add&url='+$(url).val()+'&caption='+$(caption).val()+gget('key'), dataType: 'json',
				success: function (r) {
					if (r.error) {
						...
						$(url).parent().prepend('<img src="'+$(url).val()+'" class="imagen-preview" />')
						$('<div class="field"><label>Imagen</label><div class="input-fake"><input value="http://" type="text" class="text" /><textarea style="margin-top:10px">Descripcion de la foto</textarea><a onclick="cuenta.imagen.add(this)" class="misfotos-add">Agregar</a></div></div>').appendTo('.content-tabs.mis-fotos > fieldset');
					}
				}
			});
		},
		...
		del: function (obj, id) {
			...
			$.ajax({
				type: 'post', url: '/cuenta.php', data: 'ajax=1&save=7&action=del&id='+id+'&'+gget('key'), dataType: 'json',
...
var avatar = {
	...
	upload: function (obj) {
		if ($(obj).hasClass("local")) {
			if ($('input#file-avatar').val()) {
				...
				$.ajaxFileUpload({ url: '/upload.php', fileElementId: 'file-avatar', dataType: 'json', success: this.uploadsuccess });
			}
	...
	crop: function (img) {
		...
		mydialog.body('<img class="avatar-crop" src="/avatares/'+img+'?'+Math.random()+'" onload="mydialog.center()">');
		mydialog.buttons(true, true, 'Guardar', 'avatar.save()', true, false, true, 'Cancelar', 'avatar.close()', true, true);
		$('img.avatar-big').attr('src', '/avatares/'+img+'?'+Math.random()).bind('load', function(){ $('img.avatar-crop').Jcrop({ aspectRatio: 1, sideHandles: false, setSelect: [ 0, 0, 50, 50 ], onChange: avatar.preview, onSelect: function(c) { avatar.crop_coord = c; } }) });
	},
	reload: function () {
	...
	save: function () {
		...
		else $.getJSON('/upload.php?save=true&id='+this.uid+'&s='+this.server+'&crc='+this.crc+'&coords='+this.crop_coord.x+','+this.crop_coord.y+','+this.crop_coord.x2+','+this.crop_coord.y2+'&callback=?');
	},
