...
/* Filtro Favoritos */
var filtro_favs_categoria = -1;
var filtro_favs_orden = 'guardado';
function filtro_favs(accion, value, obj){
	mydialog.close();
	if(accion=='orden'){
		filtro_favs_orden = value;
		$('#resultados .linksList thead th a').removeClass('here');
		$(obj).addClass('here');
	}else{ //Categoria
		$('.categoriaList li#cat_' + filtro_favs_categoria).removeClass('here');
		$('.categoriaList li#cat_' + value).addClass('here');
		filtro_favs_categoria = value;
	}

	$.ajax({
		type: 'POST',
		url: '/favoritos-buscar2.php',
		data: 'cat='+filtro_favs_categoria+'&sort_by='+filtro_favs_orden,
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
					...
					break;
				default:
					$('#resultados table tbody').html(h);
			}
		},
		error: function(){	
			mydialog.error_500("filtro_favs('"+accion+"', '"+value+"', '"+obj+"')");
		}
	});
}
...
/* Eliminar Comentario */
function borrar_com(comid){
	mydialog.close();
	$.ajax({
		type: 'POST',
		url: '/comentario-borrar.php',
		data: 'comid='+comid + gget('postid') + gget('key'),
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
					mydialog.alert('Error', h.substring(3));
...
$(document).ready(function(){
	...
	$('div.notificaciones-list > ul > li').live('click', function(e){
		if ($.browser.webkit && e.which == 2) return false;
		if ($(this).children('a.obj').length) location.href = $(this).children('a.obj').attr('href');
	});
});
...
/* $(document).ready(function(){
	...
	$('div.avatar-box').live('mouseover', function(e){
		if (!$(this).data('init')) {
			$(this).data('init', true);
			$(this).hoverIntent(function(){
				if ($(this).attr('userid')) {
					var obj = this, id = $(this).attr('userid');
					var cache_id = 'following_'+id;
					$(obj).children('ul').show();
					if (typeof notifica.cache[cache_id] == 'undefined') {
						notifica.ajax(Array('action=following', 'type=user', 'obj='+id), function (r) {
							r = parseInt(r);
							notifica.cache[cache_id] = r;
							notifica.userMenuPopup(obj);
						});
					}
					else notifica.userMenuPopup(obj);
				}
